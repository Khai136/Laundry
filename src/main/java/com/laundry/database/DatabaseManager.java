...
public class DatabaseManager {
    ...
    private static void executeSQLScript() {
        ...
        try (Connection conn = getConnection()) {
            ...
            for (String statement : statements) {
                if (!statement.trim().isEmpty()) {
                    try (PreparedStatement pstmt = conn.prepareStatement(statement)) {
                        ...
                        // Ignore errors for ALTER TABLE if constraint already exists
            ...
            // Hanya insert sample data jika tabel masih kosong
    ...
    /**
     * Insert sample/default data HANYA jika tabel masih kosong.
     * Ini mencegah data lama yang dihapus user muncul kembali saat restart.
     */
    private static void insertSampleDataIfEmpty(Connection conn) throws SQLException {
        // Insert default admin user (pakai INSERT IGNORE karena username UNIQUE)
        ...
        // Insert sample paket layanan HANYA jika tabel service_packages masih kosong
        int packageCount = 0;
        try (Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM service_packages")) {
            if (rs.next()) packageCount = rs.getInt(1);
        }

        if (packageCount == 0) {
            String insertPackage = "INSERT INTO service_packages (package_name, description, price_per_kg, duration_days) VALUES (?, ?, ?, ?)";
            String[][] packages = {
            ...
            System.out.println("Sample paket layanan berhasil ditambahkan.");
        } else {
            System.out.println("Tabel service_packages sudah berisi data, sample data tidak ditambahkan.");
        }

        // Insert sample customers HANYA jika tabel customers masih kosong
        int customerCount = 0;
        try (Statement stmt = conn.createStatement();
        ...
        if (customerCount == 0) {
            String insertCustomer = "INSERT INTO customers (name, phone, address, email) VALUES (?, ?, ?, ?)";
            String[][] customers = {
            ...
            System.out.println("Sample pelanggan berhasil ditambahkan.");
        } else {
            System.out.println("Tabel customers sudah berisi data, sample data tidak ditambahkan.");
        }
    }
